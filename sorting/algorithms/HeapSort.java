...
public class HeapSort extends SortingAlgorithm {
    private <T extends Comparable<T>> void heapify(T[] values,
        ...
        this.addState(states, new State<T>(values.clone(), rootIndex));
        ...
        if (largestIndex != rootIndex) {
            this.addState(states, new State<T>(values.clone(), rootIndex, largestIndex));
    ...
    public <T extends Comparable<T>> ArrayList<State<T>> states(T[] values) {
        ...
        for (int i = values.length - 1; i > 0; i--) {
            this.addState(states, new State<T>(values.clone(), 0, i));
            this.swap(values, 0, i);
